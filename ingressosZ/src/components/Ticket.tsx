...
function Ticket({ ticket }: TicketProps) {
  ...
  return (
    <div
      className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-md hover:shadow-lg transition-all duration-300 p-6 max-w-sm mx-auto relative overflow-hidden group ${
        !isValid ? "opacity-80" : ""
      }`}
    >
      {/* Status Banner */}
      <div
        className={`absolute top-0 right-0 px-4 py-1 rounded-bl-lg font-bold text-xs text-white ${statusColor}`}
      >
        {statusText}
      </div>
      ...
      <div className="space-y-4 mb-6 text-sm">
        <div className="flex items-center text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700/50 p-2 rounded">
          <div>
            ...
            <div>
              {ticket.eventDate || "Data não disponível"}{" "}
        ...
        <div className="flex items-center text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700/50 p-2 rounded">
          <div>
            ...
            <div>
              {ticket.ticketType} • R$ {ticket.price.toFixed(2)}
            </div>
          </div>
        </div>
      </div>
