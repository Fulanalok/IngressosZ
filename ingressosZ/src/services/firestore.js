...
export const ticketService = {
    ...
    async getTicketsByEvent(eventId) {
        const q = query(collection(db, "tickets"), where("eventId", "==", eventId), orderBy("purchaseDate", "desc"));
        const snapshot = await getDocs(q);
        return snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
    },
};
...
export const paymentService = {
    ...
    async getPaymentsByEvent(eventId) {
        const q = query(collection(db, "paymentSessions"), where("eventId", "==", eventId), where("status", "==", "approved"), orderBy("createdAt", "desc"));
        const snapshot = await getDocs(q);
        return snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
    },
    subscribeToAllPayments(onUpdate, onError) {
        const q = query(collection(db, "paymentSessions"), where("status", "==", "approved"), orderBy("createdAt", "desc"));
